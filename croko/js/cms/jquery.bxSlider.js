	...
	$.fn.bxSlider = function(options){		
		...
		this.goToSlide = function(number, stopAuto){
			if(!isWorking){
				...
				if(options.mode == 'horizontal'){
					$parent.animate({'right': '-'+getSlidePosition(slide, 'right')+'px'}, options.speed, options.easing, function(){
						isWorking = false;
			...
			if(!options.infiniteLoop){
				...
				if(!isWorking){
					...
					if(options.mode == 'horizontal'){						
						...
						$parent.animate({'right': '-='+parentLeft+'px'}, options.speed, options.easing, function(){
							isWorking = false;
							// if its time to loop, reset the $parent
							if(slideLoop){
								$parent.css('right', '-'+getSlidePosition(currentSlide, 'right')+'px');
							}
							// perform the callback function
							options.onAfterSlide(currentSlide, $children.length, $children.eq(currentSlide));
						});
		...
		this.startTicker = function(changeText){
			if(options.mode == 'horizontal'){
				if(options.tickerDirection == 'next'){
					...
					var stoppedLeft = parseInt($parent.css('right'));
		...
		function initCss(){
			...
			if(options.mode == 'horizontal'){
				...
				.css({
				  width: '999999px',
				  position: 'relative',
					right: '-'+(options.infiniteLoop ? origLeft : 0)+'px'
				});
		...
		function setAutoInterval(){
			if(options.auto){
				...
				if(options.tickerDirection == 'prev' && options.mode == 'horizontal'){
					$parent.css('right', '-'+(origShowWidth+origLeft)+'px');
		...
		function moveTheShow(leftCss, distance, speed){
			...
			if(options.mode == 'horizontal'){
				// if next
				if(options.tickerDirection == 'next'){
					$parent.animate({'right': '-='+distance+'px'}, speed, 'linear', function(){
						$parent.css('right', leftCss);
