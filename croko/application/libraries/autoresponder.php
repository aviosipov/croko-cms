...
class Autoresponder {
	...
	function reply_name($reply_name = NULL)
	{
	...
	function send($autoresponder_name = NULL) 
	{
		...
		if (!$this->dump && (!$autoresponder_name || !$this->to_name  )) // check to see that we received some parameters
		{
			return FALSE;
		}
		...
		$email_list =  explode(';', $this->to_email ); // convert 'avi@test.com;dani@test.com' to array 
														// before sending
		...
		if ($this->reply_email && $this->reply_name)  
		$this->CI->email->reply_to($this->reply_email, $this->reply_name);			
		...
		// $this->CI->autoresponder_model->log_email($autoresponder_log); /// not really needed ...
